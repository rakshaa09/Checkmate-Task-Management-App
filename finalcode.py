...
def list_update():
    ...
    for i, (task, completed) in enumerate(tasks):
        ...
        checkbox = tk.Checkbutton(
            ...
            font=("Times New Roman", 15),  # Set font size to 20
...
if __name__ == "__main__":
    ...
    guiWindow.geometry("700x450+300+100")
    guiWindow.resizable(0, 0)
    guiWindow.configure(bg="#81BFDA")
    ...
    # Creating frames
    task_listbox_frame = tk.Frame(guiWindow, relief="sunken", bd=2, width=200)
    functions_frame = tk.Frame(guiWindow, bg="#B1F0F7", width=650)

    tasklist_bg = "#F2F9FF"
    task_listbox_frame.configure(bg=tasklist_bg)

    lists_frame = tk.Frame(guiWindow, bg="#B1F0F7", height=100)  # Top section for lists
    ...
    list_name_field = ttk.Entry(lists_frame, font=("Times New Roman", 15), width=20)
    ...
    # Listbox for lists
    list_box = tk.Listbox(lists_frame, width=30, height=20)
    list_box.pack(pady=15)
    list_box.bind('<<ListboxSelect>>', select_list)

    # Task entry and buttons
    task_field = ttk.Entry(functions_frame, font=("Times New Roman", 12), width=25,)
